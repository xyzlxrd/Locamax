...
public class Colaborador extends Pessoa {
    ...
    public Colaborador() {} //Construtor sem Parâmetros (Criar objeto vazio e implementar os dados depois com setters)
    ...
    public void listaModelo() {
        ...
        List<Modelo> modelos = dao.listarModelosDoBanco();

        if (modelos.isEmpty()) {
            System.out.println("\nNão há nenhum modelo registrado.");
            System.out.println("");
        } else {
            System.out.println("\n--- Modelos Registrados ---");
    ...
    public void adicionarModelo(Modelo modelo) {
        if (modelo!=null) {
            modelos.add(modelo);
            System.out.println("\nModelo Adicionado(a): "+modelo.getNomeModelo());
            System.out.println("");
        } else {
    ...
    public boolean removerModeloPorId(int idModelo) {
        for (int i = 0; i < modelos.size(); i++) {
            if (modelos.get(i).getIdModelo() == idModelo) {
                modelos.remove(i);
                System.out.println("Modelo removido: ID "+idModelo);
            return true;
        }
    }
        System.out.println("\nModelo com ID "+idModelo+" não foi encontrado(a).");
        return false;
    }
    ...
    public void listaCategoria() {
        DAOColaborador dao = new DAOColaborador();
        List<Categoria> categorias = dao.listarCategoriasDoBanco();
    ...
    public void adicionarCategoria(Categoria categoria) {
        if (categoria!=null) {
            ...
            System.out.println("\nCategoria Adicionado(a): "+categoria.getNomeCategoria());
            System.out.println("");
        } else {
    ...
    public boolean removerCategoriaPorId(int idCategoria) {
        for (int i = 0; i < categorias.size(); i++) {
            if (categorias.get(i).getIdCategoria() == idCategoria) {
                categorias.remove(i);
                System.out.println("Categoria removida: ID "+idCategoria);
            return true;
        }
    }
        System.out.println("\nCategoria com ID "+idCategoria+" não foi encontrado(a).");
        return false;
    }
    ...
    public void listaCarro() {
        DAOColaborador dao = new DAOColaborador();
        List<Carro> carros = dao.listarCarrosDoBanco();

        if (carros.isEmpty()) {
            System.out.println("\nNão há nenhum carro registrado(a).");
        } else {
            System.out.println("\n--- Carros Registrados ---");
